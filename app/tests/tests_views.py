...
from app.forms import FeedbackForm, AnketaForm, BlogForm
...
class ViewsTest(TestCase):
    ...
    def test_blog_view(self):
        ...
        # Проверяем, что на странице присутствует текст 'Новости'
    ...
    def test_home_view(self):
        ...
        # Проверяем, что на главной странице присутствует текст 'Главная'
        self.assertContains(response, 'Главная')

    def test_anketa_view_get(self):
        # Проверяем, что страница анкеты возвращает код 200
        response = self.client.get(reverse('anketa'))
        self.assertEqual(response.status_code, 200)
        # Проверяем, что используется правильный шаблон 'app/anketa.html'
        self.assertTemplateUsed(response, 'app/anketa.html')
        # Проверяем, что на странице присутствует форма AnketaForm
        self.assertIsInstance(response.context['form'], AnketaForm)

    def test_anketa_view_post(self):
        # Проверяем, что при отправке формы анкеты возвращается код 200
        response = self.client.post(reverse('anketa'), {
        ...
        self.assertEqual(response.status_code, 200)
        # Проверяем, что на странице присутствует текст 'Дмитриев'
        self.assertContains(response, 'Дмитриев')
